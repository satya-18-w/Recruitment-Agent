...
class ResumeAnalysisAgent:
    ...
    def create_rag_vector_store(self, text):
        ...
        embeddings = GoogleGenerativeAIEmbeddings(model="text-embedding-004",api_key=self.api_key)
    ...
    def analyze_skill(self,qa_chain,skill):
        query=f"on scale of 0-10 how clearly does the candidate mention proficiency in {skill} Provide a numeric rating  first , followed by reasoning"
    ...
    def analyze_resume_weaknesses(self):
        ...
        for skill in self.analysis_result.get("missing_skills",[]):
            prompt=f"""
            You are a resume analysis expert great in analysing resume and identifying weekneses in resume.
            ...
            {{
                "weekness": "reasoning for the weekness with a concise explanation of what is missing in the resume or problematic with in 2 sentences",
    ...
    def extract_skils_from_jd(self,jd_text):
    ...
    def semantic_skill_analysis(self,resume_text,skills):
        """Anylyze the skills in the resume using semantic search"""
